...
**Backend-Agnostic by Design**: Today, on-device inference runs through `flutter_gemma` → LiteRT-LM, which is Android-native. However, AetherCore — the autonomous orchestration brain — does *not* import or depend on `flutter_gemma` at all. It accepts `callModel` as an injected function parameter (`Future<Stream<InferenceEvent>> Function(List<Message>)`). This deliberate decoupling means the entire agentic system — the security triad, all 7 tools, context management, self-correction loops — can be powered by *any* inference backend. Swap `flutter_gemma` for an **Ollama** or **llama.cpp** HTTP bridge, and the same agentic brain runs on macOS, Linux, or Windows with zero changes to the orchestration layer.

**CLI-Ready Architecture**: Because AetherCore is decoupled from the presentation layer, the agentic brain can operate headlessly — receiving text input and streaming tool executions via stdout. This means a single codebase can power both a polished mobile GUI for non-technical users *and* a terminal-based CLI agent for developers — same Gemma 4 intelligence, same Omega Fortress security, on every form factor from a ₹12,000 phone to a developer's laptop.

**The Vision**: One 2B model. One agentic brain. Every device — via the right local inference backend for each platform.
